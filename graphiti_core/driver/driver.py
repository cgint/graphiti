...
class GraphDriver(QueryExecutor, ABC):
    ...
    async def has_required_indices(self) -> bool:
        """Return whether the driver's required indices exist.

        Drivers that support index inspection should override this method; the
        default preserves compatibility for drivers without that capability.
        """
        return True
